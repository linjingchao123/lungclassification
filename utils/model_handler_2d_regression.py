...
class ModelHandler(object):
    ...
    def trainging_get_loss(self,
        ...
        for i in range(half_size_of_output_score):
            ...
            elif tmp_gt_score_1.item() < tmp_gt_score_2.item():
                # print "<", gt_score_1, gt_score_2
                rankingloss_func = nn.MarginRankingLoss(margin=0.1)
                target = t.ones(1)
                ranking_loss_sum += rankingloss_func(tmp_output_1, tmp_output_2, Variable(target.cuda()))
        ...
        for i in folder_list:
            ...
            for epoch in range(max_epoch):
                ...
                for ii, (image, gt_score, gt_label,
                    ...
                    print("Epoch {}, Step {}, " \
                          "Losses are: {:.5f}, {:.5f}, {:.5f}, " \
                          "Total loss is {:.5f}\t\t\t\t\t Sample Num: {}".format(
                        epoch, ii,
                        float(loss1.item()),
                        float(rankingloss if type(rankingloss) == float else rankingloss.item()),
                        float(loss2.item()),
                        float(loss.item()),
                        image.shape[0]))
                    ...
                    if ii % training_params.get("print_freq") == training_params.get("print_freq") - 1:
                        ...
                        writer.add_scalars("data/loss", {"train": float(loss.item())}, training_n_iter)
    ...
    def val(self, dataloader, training_params, criterion):
        ...
        for ii, (image, gt_score, gt_label,
            ...
            accuracy_1 = self.testing_get_p_val(
                np.squeeze(cat_internalStructure_score.cpu().data.numpy(), axis=1).tolist(),
        ...
        for i in reversed(range(k_folder)):
            ...
            for model_path in model_path_list:
                ...
                for ii, (data, gt_score, ground_truth,
                    ...
                    total_score = t.unsqueeze(output_score_cuda.mean(dim=1), 0)
                    cat_subtlety_score = t.nn.functional.sigmoid(cat_subtlety_score)
                    cat_internalStructure_score = t.nn.functional.sigmoid(cat_internalStructure_score)
                    cat_calcification_score = t.nn.functional.sigmoid(cat_calcification_score)
                    cat_sphericity_score = t.nn.functional.sigmoid(cat_sphericity_score)
                    ...
                    total_probability = t.nn.functional.softmax(output_score_cuda)
                    ...
                    colume_name_2 = list(list(zip(*other_info_name_2))[0])
                    ...
                    confusion_matrix.add(output_score_cuda.max(dim=1)[1].data, ground_truth)
                ...
                model_number_for_hard_example = "_".join(model_path.split("/")[-1].split("_")[:2]) \
